...
import { Eye, EyeOff, Loader2, CheckCircle, GraduationCap, ArrowLeft } from "lucide-react";
...
export default function TrainerLoginPage() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      console.log('⏰ Trainer login timeout - forcing render');
  ...
  useEffect(() => {
    console.log('🔄 Trainer login useEffect:', { isMounted, status, session: session ? 'Present' : 'None' });
    ...
    } else if (session && session.user?.role !== 'trainer') {
      console.log('❌ Non-trainer user detected, redirecting to appropriate portal');
      // Redirect non-trainer users to their appropriate portal
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('🚀 Starting trainer login process...');
  ...
  if (!isMounted || (status === 'loading' && isMounted)) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-gray-900 dark:to-gray-800 p-4">
      ...
      <Card className="mx-auto w-full max-w-md shadow-lg rounded-xl border-0 dark:border dark:border-gray-700">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="email"
                type="email"
                placeholder="trainer@tunalismus.com"
                required
                value={email}
          ...
          <div className="mt-4 text-center text-sm">
            <Link href="/" className="text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300 flex items-center justify-center">
