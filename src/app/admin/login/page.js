...
import { Eye, EyeOff, Loader2, CheckCircle, Shield, ArrowLeft } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      console.log('⏰ Admin login timeout - forcing render');
  ...
  useEffect(() => {
    console.log('🔄 Admin login useEffect:', { isMounted, status, session: session ? 'Present' : 'None' });
    ...
    if (session && session.user?.role === 'admin') {
      ...
      console.log('❌ Non-admin user detected, redirecting to appropriate portal');
      ...
      <Card className="mx-auto w-full max-w-md shadow-lg rounded-xl border-0 dark:border dark:border-gray-700">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="email"
                type="email"
                placeholder="admin@tunalismus.com"
                required
                value={email}
            ...
            <Button 
              type="submit" 
              className="w-full h-11 rounded-lg bg-red-600 hover:bg-red-700 
          ...
          <div className="mt-4 text-center text-sm">
            <Link href="/" className="text-red-600 hover:text-red-500 dark:text-red-400 dark:hover:text-red-300 flex items-center justify-center">
